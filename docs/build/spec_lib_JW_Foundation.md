...
> ## 🔴 تصحيح حاكم (المنشور `JW_Foundation/2` — يَنسخ التواقيع القديمة أدناه)
> - **`method step` (§1/§3):** ❌ لا يوجد. المنفَّذ = **`legInputs(self, waveScaleDays, legMinRatio)`** (يطوي `ta.*` + الهيستيريسيس + `bigDir`، يُستدعى كلّ بار بلا شرط) + **`legStep(...)`** (دالّة تُرجِع `dir0`). الهيستيريسيس داخل `legInputs` لا `step`.
> - **`type LegEngine`:** يشمل حقل **`bigDir`** (اتجاه الموجة العامّ) بعد المصفوفات. **`buildSegments`** يأخذ معامل **`useHohN`**.
> - **حارس MAX_HIST=4900 (v2):** ثابت رأسيّ واحد (يوحّد الـ4900 المكرَّرة في rangeHiLo/legInputs/buildSegments)؛ **`rangeDays` تُرجِع `na`** إن `(bar_index-leftB)>MAX_HIST` أو مدخلات مقلوبة/na (بدل الوصول غير المحروس ⇒ لا crash). يتطلّب المستهلِك `max_bars_back ≥ MAX_HIST+1`.
